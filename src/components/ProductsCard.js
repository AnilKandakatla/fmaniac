...
import { addToCart } from '../redux/fashionSlice';
...
const ProductsCard = ({product}) => {
  ...
  return (
    <div className="group relative bg-black text-slate-200">
      ...
      <div className="w-full border-[1px] border-slate-100 px-2 py-4">
        ...
        <div className="flex gap-2 justify-end relative overflow-hidden w-28 text-sm">
          <div className="flex gap-2 transform group-hover:translate-x-24 trasition-transform duration-500">
          <p className="line-through text-gray-500">${product.oldPrice}</p>
          <p className="font-semibold">${product.price}</p>
          </div>
          <p onClick={() => dispatch(addToCart({
            _id: product._id, title: product.title, image: product.image, price: product.price,
            quantity: 1, description: product.description
          })) & toast.success(`${product.title} is added `)
        } className="absolute z-20 w-[100px] text-slate-200 hover:text-slate-200 flex items-center
